...
use super::{UiContainer, Position, Size};
use crate::ui::{Dialog, Label, Button, Drawable};
use crate::game::GameRef;
use crate::game::game_state::{GameState, GameStateUpdateResult, GameStateResult, GameStateChange};
use std::borrow::Cow;
...
impl MessageBox {
    ...
    pub fn game_state<S: AsRef<str>>(game_ref: &mut GameRef, text: S, first_button_text: S, first_result: Box<dyn FnOnce() -> GameStateChange>, second_button_text: Option<S>, second_result: Option<Box<dyn FnOnce() -> GameStateChange>>) -> Box<dyn GameState> {
        Box::new(Self::create_message_box(game_ref, text, first_button_text, first_result, second_button_text, second_result))
    }

    pub fn create_message_box<S: AsRef<str>>(game_ref: &mut GameRef, text: S, first_button_text: S, first_result: Box<dyn FnOnce() -> GameStateChange>, second_button_text: Option<S>, second_result: Option<Box<dyn FnOnce() -> GameStateChange>>) -> Self {
        ...
        loop {
            ...
            contents.add_component(Box::new(Label::new(&text[i..j], Position::new(27, (2 + (i / Self::MAX_TEXT_LINE_LENGTH)) as i32))));
        ...
        match &second_button_text {
            None => {
                contents.add_component(Box::new(Button::new(first_button_text.clone(), Position::new(22, (i / Self::MAX_TEXT_LINE_LENGTH + 3) as i32), MessageBox::BUTTON_WIDTH, first_button_shortcut, true)));
            }
            Some(second_button_text) => {
                contents.add_component(Box::new(Button::new(first_button_text.clone(), Position::new(8, (i / Self::MAX_TEXT_LINE_LENGTH + 3) as i32), MessageBox::BUTTON_WIDTH, first_button_shortcut, true)));

                let shortcut = second_button_text.to_lowercase().chars().next();
                contents.add_component(Box::new(Button::new(second_button_text.clone(), Position::new(31, (i / Self::MAX_TEXT_LINE_LENGTH + 3) as i32), MessageBox::BUTTON_WIDTH, shortcut, true)));
            }
        }
...
impl GameState for MessageBox {
    ...
    fn update(&mut self, game_ref: &mut GameRef) -> GameStateUpdateResult {
        let result = self.dialog.update(Position::default(), game_ref.input.clone());
